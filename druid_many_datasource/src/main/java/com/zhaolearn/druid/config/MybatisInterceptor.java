package com.zhaolearn.druid.config;
...
public class MybatisInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (SqlCommandType.INSERT == sqlCommandType) {
			Field[] fields = com.zhaolearn.druid.config.oConvertUtils.getAllFields(parameter);
				...
				try {
					...
					if ("createBy".equals(field.getName())) {
						...
						if (local_createBy == null || local_createBy.equals("")) {
							...
							// 获取登录用户信息（通常通过登录用户获取信息的，具体系统具体写）（这里使用默认值zhao）
							//UserInfo sysUser = CurrentUserInfoUtils.getUserInfo();
//							if (sysUser != null) {
//								// 登录账号
//								createBy = sysUser.getUsername();
//							}
							if (com.zhaolearn.druid.config.oConvertUtils.isNotEmpty(createBy)) {
								field.setAccessible(true);
		...
		if (SqlCommandType.UPDATE == sqlCommandType) {
			...
			if (parameter instanceof ParamMap) {
				...
				fields = com.zhaolearn.druid.config.oConvertUtils.getAllFields(parameter);
			} else {
				fields = com.zhaolearn.druid.config.oConvertUtils.getAllFields(parameter);
			}
...
//							}
							if (com.zhaolearn.druid.config.oConvertUtils.isNotEmpty(updateBy)) {
								field.setAccessible(true);
